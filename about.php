<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="style.css">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Cardo&display=swap" rel="stylesheet">
</head>
<body>
<?php 
    include("subpageHeader.php");
  ?>
        <div class="aboutusbox">
            <h2 class="aboutus">ABOUT US</h2>
            <p class="aboutusparagraph">We are a group of friends who are big foodies and 
                  grew up in China. Being away from home made us going on this food making journey.
                  Some of us went too culinary schools in China to learn the required skills and put all 
                  our love and effort to this place. At Taste of Home, we want you to have the most authentic 
                  experience as well as taste like what your mom would serve you at home. 
            </p>
            <span class="aboutmeparagraph">How we describe us:</span>
            
                 <p>Authentic </p>
                 <p>Not your typical Chinese food </p>
                 <p>Comfort as if you are at home</p>
            
         
            <span class="aboutmeparagraph" > How others describe us: </span>
          
                 <p>Amazing</p>
                 <p class="mylabels">Mind blowing delicious</p>
                 <p class="mylabels">Homemade is the best</p>
                 <p class="mylabels">Made with love</p>
            
        </div>
        
        <section class="aboutgridbox">
            <div class="aboutlist">
                
                <img class="gridimage" src="../Web-Assignment02/Pictures/dandanmian.jpg" alt="database image">
                <p></p>
            </div>
            <div class="aboutlist">
                
                <img class="gridimage" src="../Web-Assignment02/Pictures/jianjiao.jpg" alt="linux image">
                <p></p>
            </div>
            <div class="aboutlist">
                
                <img class="gridimage" src="../Web-Assignment02/Pictures/mochi.jpg" alt="java image">
                <p></p>
            </div>
            <div class="aboutlist">
               
                <img class="gridimage" src="../Web-Assignment02/Pictures/spice.jpg" alt="web image">
                <p></p>
            </div>
            <div class="aboutlist">
                
                <img class="gridimage" src="../Web-Assignment02/Pictures/zongzi.jpg" alt="math image">
                <p></p>
            </div>
            <div class="aboutlist">
               
                <img class="gridimage" src="../Web-Assignment02/Pictures/doufubowl.jpg"
                    alt="computer essential image">
                <p></p>
            </div>
            <div class="aboutlist">
        
                <img class="gridimage" src="../Web-Assignment02/Pictures/friedrice.jpg"
                    alt="computer programming image">
                <p></p>
            </div>
            <div class="aboutlist">
               
                <img class="gridimage" src="../Web-Assignment02/Pictures/jianjiao2.jpg" alt="database aboutlist image">
                <p></p>
            </div>
            <div class="aboutlist">
               
                <img class="gridimage" src="../Web-Assignment02/Pictures/shamianfen.jpg"
                    alt="communication aboutlist image">
                <p></p>
            </div>

            <div class="aboutlist">
               
                <img class="gridimage" src="../Web-Assignment02/Pictures/baojiaozi.jpg"
                    alt="communication aboutlist image">
                <p></p>
            </div>
        </section>

<footer class="footer">
  <?php 
      include("footer.php");
    ?>
</footer>

</body>
</html>